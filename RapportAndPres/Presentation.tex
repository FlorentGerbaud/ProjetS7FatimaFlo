...
\begin{document}
	...
	Here the objectif is like before it ean study the stbility of the model. The equation in 1D is the folowing one : 
	...
	The problem with this equation is that the Anaytical solution is impossible to find. So to Study the equilibrium we are going to use the indirect Theorem of Lyupanov.
	The first step in 1D is to calculate $f'(d_1,t)$. XWe find easily : 
	\begin{align*}
		f'(d_1,t)=-\alpha_2e^{-\frac{\alpha_2}{V_2}(d_1 - d_{2}^{sec})}
	\end{align*}
	
	We could easily see that $f'(d_1,t)<0 \forall \alpha_2>0$. So the equilibrium is in our context everytime stable because we never take acceleration negative
