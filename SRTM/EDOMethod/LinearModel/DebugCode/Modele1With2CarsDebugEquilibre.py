...
for t in range(1, n_steps):
    ...
    d1Tab[t] = d1Tab[t-1] + d1_prime(t-1) * h
    time[t] = time[t-1] + h
    if(x2Pos[t] >= x1Pos[t]):
        accident=True
        break;
...
if(accident==True):
    ...
    plt.plot(time[0:t+1], d1Tab[0:t+1], label='d1(t)')
    plt.xlabel('Time')
    plt.ylabel('Values')
    plt.legend()
    ...
    plt.plot(time, d1Tab, label='d1(t)')
    plt.xlabel('Time')
    plt.ylabel('Values')
    plt.legend()
